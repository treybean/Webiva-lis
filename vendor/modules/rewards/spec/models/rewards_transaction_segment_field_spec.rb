...
describe RewardsTransactionSegmentField do
  ...
  it "should only have valid RewardsUser fields" do
    ...
    obj = RewardsTransactionSegmentField.user_segment_fields_handler_info[:domain_model_class].new
    RewardsTransactionSegmentField.user_segment_fields.each do |key, value|
  ...
  it "can output field data" do
    handler_data = RewardsTransactionSegmentField.get_handler_data([@user1.id, @user2.id], [:notification])

    RewardsTransactionSegmentField.user_segment_fields.each do |key, value|
      next if value[:search_only]
      RewardsTransactionSegmentField.field_output(@user1, handler_data, key)
    end
  end
  ...
  it "should be able to sort on sortable fields" do
    ...
    RewardsTransactionSegmentField.user_segment_fields.each do |key, value|
      ...
      scope = RewardsTransactionSegmentField.sort_scope(key.to_s, 'DESC')
